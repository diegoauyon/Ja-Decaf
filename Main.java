...
public class Main implements GPMessageConstants
{
	...
	public Main(String textToParse, IDecaf padre)
    {
        ...
        boolean done = false;
        boolean pasoAntes = false;
        int response = -1;
        Expresion expDe = null;
 	    int numOperacion = -1;
        String nombreVarSig = "";
        if ( (textToParse != null))  
        {
	                    ...
	                    switch(parser.currentReduction().getParentRule().getTableIndex())
	                    {
	                       ...
	                       case RuleConstants.RULE_RELEXP:
	                          //<relExp> ::= <sumaExp> <rel_op> <relExp>
	                    	   ...
	                    	   expDe = (Expresion)stack.pop();
	                    	   numOperacion = (Integer)stack.pop();
	                    	   if (stack.peek().getClass() == String.class)
	                    	   {
	                    		   Location loc = new Location((String)stack.pop());
	                    		   stack.push(new ExpresionBinaria(expDe,numOperacion,loc));
	                    	   }
	                    	   else
	                    	   {
	                       ...
	                       case RuleConstants.RULE_LOCATION1_ID:
	                          //<location1> ::= id <location2>
	                    	   stackPruebas(stack.clone(), "Location1");
	                    	/**   if (stack.peek().getClass() == LinkedList.class)
	                    	   {
	                    		   LinkedList a = (LinkedList) stack.pop();
	                    		   stack.push(new Location((String)stack.pop()));
	                    		   stack.push(a);
	                    	   }
	                    	   else
	                    	   {
	                       ...
	                       case RuleConstants.RULE_INT_LITERAL_NUM:
	                          //<int_literal> ::= num
	                    	   ...
	                    	   {
	                    		   int numCons = (Integer)stack.pop();
	                    		   stack.push(new Literal(""+(Integer)stack.pop(),Const.TipoLiteral.integer));
	                    		   Literal lita = (Literal) stack.pop();
	                    		   stack.push(numCons);
	                    		   stack.push(lita);
	                    	   }
	                          break;
